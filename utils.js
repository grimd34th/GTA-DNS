const DOMAIN_ATV = "prod.cloud.rockstargames.com";
const DOMAIN_ATV2 = "prod.ros.rockstargames.com"
...
/*        var options = {
	...
	var server = http.createServer( function(request, response) {
		...
		fs.stat(realPath, function(err, stats) {
			if (err) {
				var string;
				https.get('https://prod.ros.rockstargames.com'+pathname,function(response1) {
					response1.on('data', function(d) { string += d; });
...
function startDnsProxy(localIp) {
	...
	dgram.createSocket("udp4", function(msg, rinfo) {
		...
		if (domain == DOMAIN_ATV || domain == DOMAIN_ATV2) {
